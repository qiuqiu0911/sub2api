...
export function useKiroOAuth() {
  ...
  const error = ref('')
  const deviceAuthorization = ref(false)
  ...
  const resetState = () => {
    ...
    loading.value = false
    error.value = ''
    deviceAuthorization.value = false
  ...
  const generateIDCAuthUrl = async (
    ...
    try {
      const response = await adminAPI.kiro.startIDCDeviceAuthorization({
      ...
      sessionId.value = response.session_id
      state.value = ''
      deviceAuthorization.value = true
      deviceUserCode.value = response.user_code
      return true
    } catch (err: any) {
      error.value = err.response?.data?.detail || t('admin.accounts.oauth.authFailed')
      appStore.showError(error.value)
  ...
  const pollIDCDeviceToken = async (): Promise<KiroTokenInfo | null> => {
    const currentSessionID = sessionId.value
    if (!currentSessionID || !deviceAuthorization.value) return null
    ...
    error.value = ''
    try {
      while (generation === devicePollGeneration) {
        await new Promise((resolve) => window.setTimeout(resolve, retryAfterSeconds * 1000))
        if (generation !== devicePollGeneration) return null

        const response = await adminAPI.kiro.pollIDCDeviceToken(currentSessionID)
        if (response.status === 'success' && response.token_info) {
          return response.token_info
        }
        if (response.status === 'expired' || response.status === 'denied') {
          error.value = response.status === 'expired'
            ? '设备授权已过期，请重新生成授权链接'
            : '设备授权已被拒绝'
          appStore.showError(error.value)
          return null
        }
        retryAfterSeconds = Math.max(response.retry_after_seconds || retryAfterSeconds, 1)
      }
      return null
    } catch (err: any) {
      error.value = err.response?.data?.detail || t('admin.accounts.oauth.authFailed')
      ...
      if (generation === devicePollGeneration) {
        devicePolling.value = false
      }
    }
  }

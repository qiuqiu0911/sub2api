  ...
  >
    ...
    <div v-else class="space-y-5">
      ...
      <OAuthAuthorizationFlow
        ...
        :device-authorization="form.platform === 'kiro' && kiroAccountType === 'idc' && kiroOAuth.deviceAuthorization.value"
        :device-user-code="kiroOAuth.deviceUserCode.value"
        :device-polling="kiroOAuth.devicePolling.value"
    ...
    <template #footer>
      ...
      <div v-else class="flex justify-between gap-3">
        ...
        <button
          v-else-if="isManualInputMethod && !(form.platform === 'kiro' && kiroAccountType === 'idc')"
          type="button"
...
const handleGenerateUrl = async () => {
  if (form.platform === 'openai') {
    ...
    if (kiroAccountType.value === 'idc') {
      const started = await kiroOAuth.generateIDCAuthUrl({
        proxyId: form.proxy_id,
        startUrl: kiroIDCStartUrl.value.trim() || undefined,
        region: kiroIDCRegion.value.trim() || undefined
      })
      if (!started) return
      const tokenInfo = await kiroOAuth.pollIDCDeviceToken()
      if (!tokenInfo) return
      await createAccountAndFinish('kiro', 'oauth', buildKiroCredentials(tokenInfo))
    } else {

...
describe.each([
  ['en', en],
  ['zh', zh],
] as const)('Kiro locale preservation: %s', (_name, locale) => {
  it('keeps every Kiro locale key', () => {
    expect(preservedKiroKeys.length).toBeGreaterThan(0)
    expect(preservedKiroKeys.filter((key) => localeValue(locale, key) === undefined)).toEqual([])
  })
})

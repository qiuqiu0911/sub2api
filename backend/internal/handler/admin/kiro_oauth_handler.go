...
func (h *KiroOAuthHandler) StartIDCDeviceAuthorization(c *gin.Context) {
	var req KiroGenerateIDCAuthURLRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		response.BadRequest(c, "请求无效: "+err.Error())
		return
	}
	result, err := h.kiroOAuthService.StartIDCDeviceAuthorization(
		c.Request.Context(),
		&service.KiroGenerateIDCAuthURLInput{
			ProxyID:  req.ProxyID,
			StartURL: req.StartURL,
			Region:   req.Region,
		},
	...
	if err != nil {
		response.BadRequest(c, "启动 IDC 设备授权失败: "+err.Error())
		return
	}
	response.Success(c, result)
}

func (h *KiroOAuthHandler) PollIDCDeviceToken(c *gin.Context) {
	sessionID := c.Param("session_id")
	if sessionID == "" {
		response.BadRequest(c, "session_id 不能为空")
		return
	}
	result, err := h.kiroOAuthService.PollIDCDeviceToken(
		c.Request.Context(),
		&service.KiroPollIDCDeviceTokenInput{SessionID: sessionID},
	)
	if err != nil {
		response.BadRequest(c, "查询 IDC 设备授权状态失败: "+err.Error())
		return
	}
	response.Success(c, result)
}

...
func TestBuildKiroIDERuntimeUserAgentUsesCurrentStableVersion(t *testing.T) {
	machineID := BuildMachineID("refresh-token", "", "")

	userAgent := BuildKiroIDERuntimeUserAgent("account-a", machineID)

	require.Equal(t, "KiroIDE "+defaultKiroIDEUserAgentVersion+" "+machineID, userAgent)
}

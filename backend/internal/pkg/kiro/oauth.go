...
const (
	...
	idcDeviceGrantType  = "urn:ietf:params:oauth:grant-type:device_code"
...
type IDCDeviceAuthorization struct {
...
type IDCDeviceTokenPollStatus string

const (
	IDCDeviceTokenPending  IDCDeviceTokenPollStatus = "pending"
	IDCDeviceTokenSlowDown IDCDeviceTokenPollStatus = "slow_down"
	IDCDeviceTokenExpired  IDCDeviceTokenPollStatus = "expired"
	IDCDeviceTokenDenied   IDCDeviceTokenPollStatus = "denied"
	IDCDeviceTokenSuccess  IDCDeviceTokenPollStatus = "success"
)

type IDCDeviceTokenPollResult struct {
	Status IDCDeviceTokenPollStatus
	Token  *TokenData
}
...
func RegisterIDCDeviceClient(ctx context.Context, proxyURL, issuerURL, region string) (*registerClientResponse, error) {
	if region == "" {
		region = defaultIDCRegion
	}
	payload := map[string]any{
		...
		"grantTypes": []string{idcDeviceGrantType, "refresh_token"},
		"issuerUrl":  issuerURL,
	}
	var resp registerClientResponse
	headers := oidcHeaders("", BuildMachineID("", "", "register-idc-device-client"))
	if err := doJSON(
		ctx,
		proxyURL,
		http.MethodPost,
		getOIDCEndpoint(region)+"/client/register",
		payload,
		&resp,
		headers,
	); err != nil {
...
func StartIDCDeviceAuthorization(
	ctx context.Context,
	proxyURL string,
	clientID string,
	clientSecret string,
	...
	var resp IDCDeviceAuthorization
	...
	if err := doJSON(
		...
		proxyURL,
		http.MethodPost,
		getOIDCEndpoint(region)+"/device_authorization",
		payload,
		&resp,
		headers,
	); err != nil {
...
func PollIDCDeviceToken(
	...
	region string,
	startURL string,
) (*IDCDeviceTokenPollResult, error) {
	if region == "" {
		region = defaultIDCRegion
	}
	payload := map[string]string{
		"clientId":     clientID,
		"clientSecret": clientSecret,
		"grantType":    idcDeviceGrantType,
		"deviceCode":   deviceCode,
	}
	accountKey := BuildAccountKey(clientID, "", "", "", 0)
	headers := oidcHeaders(accountKey, BuildMachineID("", "", "clientID:"+clientID))
	statusCode, responseBody, err := doJSONRaw(
		ctx,
		proxyURL,
		http.MethodPost,
		getOIDCEndpoint(region)+"/token",
		payload,
		headers,
	)
	if err != nil {
	...
	if statusCode < http.StatusOK || statusCode >= http.StatusMultipleChoices {
		var oidcError oidcErrorResponse
		if unmarshalErr := json.Unmarshal(responseBody, &oidcError); unmarshalErr == nil {
			switch strings.TrimSpace(oidcError.Error) {
			case "authorization_pending":
				return &IDCDeviceTokenPollResult{Status: IDCDeviceTokenPending}, nil
			case "slow_down":
				return &IDCDeviceTokenPollResult{Status: IDCDeviceTokenSlowDown}, nil
			case "expired_token":
				return &IDCDeviceTokenPollResult{Status: IDCDeviceTokenExpired}, nil
			case "access_denied":
				return &IDCDeviceTokenPollResult{Status: IDCDeviceTokenDenied}, nil
			}
		}
		return nil, fmt.Errorf(
			"upstream request failed (status %d): %s",
			statusCode,
	...
	return &IDCDeviceTokenPollResult{Status: IDCDeviceTokenSuccess, Token: token}, nil
}
...
func doJSON(ctx context.Context, proxyURL, method, rawURL string, payload any, out any, extraHeaders map[string]string) error {
	statusCode, respBody, err := doJSONRaw(ctx, proxyURL, method, rawURL, payload, extraHeaders)
	if err != nil {
		return err
	}
	if statusCode < 200 || statusCode >= 300 {
		bodyText := strings.TrimSpace(string(respBody))
		if statusCode == http.StatusBadRequest && strings.Contains(strings.ToLower(bodyText), "invalid_grant") {
			return &RefreshTokenInvalidError{StatusCode: statusCode, Body: bodyText}
		}
		return fmt.Errorf("upstream request failed (status %d): %s", statusCode, bodyText)
	}

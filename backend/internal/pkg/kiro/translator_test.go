...
func TestBuildKiroPayloadUsesAdaptiveThinkingForOpus5Alias(t *testing.T) {
	body := []byte(`{
	...
	require.Equal(t, "adaptive",
		gjson.GetBytes(result.Payload, "additionalModelRequestFields.thinking.type").String())
	require.Equal(t, "summarized",
		gjson.GetBytes(result.Payload, "additionalModelRequestFields.thinking.display").String())
	require.Equal(t, "high",
		gjson.GetBytes(result.Payload, "additionalModelRequestFields.output_config.effort").String())
}

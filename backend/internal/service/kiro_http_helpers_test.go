...
func TestKiroProxyURLUsesEnvironmentProxy(t *testing.T) {
	t.Setenv("HTTPS_PROXY", "http://127.0.0.1:7897")
	t.Setenv("https_proxy", "")
	t.Setenv("NO_PROXY", "")
	t.Setenv("no_proxy", "")

	require.Equal(t, "http://127.0.0.1:7897", kiroProxyURL(nil))
}

func TestKiroProxyURLPrefersAccountProxy(t *testing.T) {
	t.Setenv("HTTPS_PROXY", "http://127.0.0.1:7897")
	proxyID := int64(7)
	account := &Account{
		ProxyID: &proxyID,
	...
	require.Equal(t, "http://10.0.0.8:8080", kiroProxyURL(account))
}
...
func TestNewKiroJSONRequestAddsConditionalHeaders(t *testing.T) {
	...
	require.Empty(t, req.Header.Get("TokenType"))
...
func TestApplyKiroConditionalHeadersOAuthTokenTypes(t *testing.T) {
	testCases := []struct {
		name       string
		authMethod string
		want       string
	}{
		{name: "enterprise idc", authMethod: "idc", want: "SSO_OIDC"},
		{name: "explicit sso oidc", authMethod: "SSO_OIDC", want: "SSO_OIDC"},
		{name: "external identity provider", authMethod: "external_idp", want: "EXTERNAL_IDP"},
		{name: "machine token", authMethod: "machine_token", want: "KIRO_MACHINE_TOKEN"},
	}
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			account := &Account{
				...
				Type:     AccountTypeOAuth,
				Credentials: map[string]any{
					"auth_method": testCase.authMethod,
				},
			}
			req, err := newKiroJSONRequest(
				context.Background(),
			...
			require.NoError(t, err)
			require.Equal(t, testCase.want, req.Header.Get("TokenType"))
		})
	}
}

func TestApplyKiroConditionalHeadersQOmitsOAuthTokenType(t *testing.T) {
	account := &Account{
		Platform: PlatformKiro,
		Type:     AccountTypeOAuth,
		Credentials: map[string]any{
			"auth_method": "idc",
			"profile_arn": "arn:aws:codewhisperer:us-east-1:123456789012:profile/IDC",
		},
	}

...
type KiroIDCDeviceAuthorizationResult struct {
	AuthURL         string `json:"auth_url"`
	SessionID       string `json:"session_id"`
...
type KiroIDCDevicePollResult struct {
...
type KiroPollIDCDeviceTokenInput struct {
	SessionID string
}
...
func (s *KiroOAuthService) StartIDCDeviceAuthorization(
	ctx context.Context,
	input *KiroGenerateIDCAuthURLInput,
) (*KiroIDCDeviceAuthorizationResult, error) {
	startURL := strings.TrimSpace(input.StartURL)
	if startURL == "" {
		startURL = kiropkg.BuilderIDStartURL
	}
	...
	proxyURL, _ := s.resolveProxyURL(ctx, input.ProxyID)
	registration, err := kiropkg.RegisterIDCDeviceClient(ctx, proxyURL, startURL, region)
	if err != nil {
		return nil, err
	}
	authorization, err := kiropkg.StartIDCDeviceAuthorization(
		ctx,
		proxyURL,
	...
	if strings.TrimSpace(authorization.DeviceCode) == "" {
		return nil, fmt.Errorf("kiro IDC device authorization returned empty device code")
	}
	...
	authURL := strings.TrimSpace(authorization.VerificationURIComplete)
	if authURL == "" {
		authURL = strings.TrimSpace(authorization.VerificationURI)
	}
	return &KiroIDCDeviceAuthorizationResult{
		AuthURL:         authURL,
		SessionID:       sessionID,
		UserCode:        authorization.UserCode,
		VerificationURI: authorization.VerificationURI,
		ExpiresAt:       createdAt.Add(time.Duration(expiresIn) * time.Second).Format(time.RFC3339),
...
func (s *KiroOAuthService) PollIDCDeviceToken(
	ctx context.Context,
	input *KiroPollIDCDeviceTokenInput,
) (*KiroIDCDevicePollResult, error) {
	session, ok := s.sessionStore.Get(strings.TrimSpace(input.SessionID))
	if !ok {
		return &KiroIDCDevicePollResult{Status: string(kiropkg.IDCDeviceTokenExpired)}, nil
	}
	if session.AuthType != "idc_device" {
		return nil, fmt.Errorf("invalid kiro IDC device authorization session")
	}
	result, err := kiropkg.PollIDCDeviceToken(
		ctx,
		session.ProxyURL,
		session.ClientID,
	...
	retryAfter := int(session.PollInterval / time.Second)
	if retryAfter <= 0 {
		retryAfter = 5
	}
	if result.Status == kiropkg.IDCDeviceTokenSlowDown {
		retryAfter += 5
		session.PollInterval = time.Duration(retryAfter) * time.Second
		s.sessionStore.Set(input.SessionID, session)
	}
	if result.Status != kiropkg.IDCDeviceTokenSuccess {
		if result.Status == kiropkg.IDCDeviceTokenExpired || result.Status == kiropkg.IDCDeviceTokenDenied {
			s.sessionStore.Delete(input.SessionID)
		}
		return &KiroIDCDevicePollResult{
	...
	return &KiroIDCDevicePollResult{
		Status:    string(result.Status),
		TokenInfo: toKiroTokenInfo(result.Token),
	}, nil
}

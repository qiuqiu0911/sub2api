...
func (s *AccountTestService) executeKiroTestUpstream(ctx context.Context, account *Account, anthropicBody []byte, mappedModel, token string) (*http.Response, error) {
	...
	currentToken := token
	// 测试连接对齐 Kiro IDE 的 Q endpoint：Enterprise/IdC 需要根级真实 profileArn，
	// API Key 和 BuilderID 占位符仍保持不注入。
	profileArn := resolveKiroQProfileArn(account)
	if profileArn == "" && strings.EqualFold(
		strings.TrimSpace(account.GetCredential("provider")),
		kiropkg.ProviderEnterprise,
	) {
		_ = kiroResolveAndPersistProfileArn(ctx, s.accountRepo, account, currentToken)
		profileArn = resolveKiroQProfileArn(account)
	}
